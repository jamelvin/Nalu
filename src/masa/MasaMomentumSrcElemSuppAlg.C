...
// Class Definition
//==========================================================================
// MasaMomentumSrcElemSuppAlg - base class for algorithm
//==========================================================================
//--------------------------------------------------------------------------
//-------- constructor -----------------------------------------------------
//--------------------------------------------------------------------------
MasaMomentumSrcElemSuppAlg::MasaMomentumSrcElemSuppAlg(
  Realm &realm)
...
//--------------------------------------------------------------------------
void
MasaMomentumSrcElemSuppAlg::elem_resize(
  MasterElement */*meSCS*/,
  MasterElement *meSCV)
{
  ...
  if (masaStore_) ws_momentumSrc_.resize(nDim_*nodesPerElement);
...
//--------------------------------------------------------------------------
void
MasaMomentumSrcElemSuppAlg::setup()
{
  // nothing
}
...
//--------------------------------------------------------------------------
void
MasaMomentumSrcElemSuppAlg::elem_execute(
  double */*lhs*/,
  double *rhs,
  ...
  MasterElement *meSCV)
{
  ...
  for ( int ni = 0; ni < num_nodes; ++ni ) {
    ...
    if (masaStore_) momentumSrc = stk::mesh::field_data(*momentumSrc_, node );
    // gather vectors
    const int niNdim = ni*nDim_;
    for ( int j=0; j < nDim_; ++j ) {
      ws_coordinates_[niNdim+j] = coords[j];
      if (masaStore_) ws_momentumSrc_[niNdim+j] = momentumSrc[j];
    }
  }
  ...
  for ( int ip = 0; ip < numScvIp; ++ip ) {
    ...
    for ( int ic = 0; ic < nodesPerElement; ++ic ) {
      ...
      for ( int j = 0; j < nDim_; ++j ) {
        ...
        if (masaStore_) scvMomSrc_[j] += r*ws_momentumSrc_[ic*nDim_+j];
      }
    }
    ...
    if (masaStore_) {
      srcXi_[0] = scvMomSrc_[0];
      srcXi_[1] = scvMomSrc_[1];

      if (nDim_ == 3) {
        srcXi_[2] = scvMomSrc_[2];
      }
    }
    else {
