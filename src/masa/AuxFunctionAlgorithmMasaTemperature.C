...
AuxFunctionAlgorithmMasaTemperature::AuxFunctionAlgorithmMasaTemperature(
  Realm & realm,
  stk::mesh::Part * part,
...
void
AuxFunctionAlgorithmMasaTemperature::execute()
{
  ...
  ScalarFieldType *tempExact_;
  if (masaStore_) tempExact_ = meta_data.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "temperatureExact");
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = buckets.begin();
    ...
    double * tempExact;
    if (masaStore_) tempExact = (double*) stk::mesh::field_data( *tempExact_, *b.begin() );
    ...
    for(unsigned p=0; p < length; ++p) {
      ...
      if (masaStore_)
      {
         fieldData[0] = tempExact[0]; 
         tempExact += fieldSize;
      }
      else
         fieldData[0] = masa_eval_3d_exact_t(x,y,z);
